...
void delay_ns (uint16_t ns)
{
	...
	while (__HAL_TIM_GET_COUNTER(&htim2) < ns);  // wait for the counter to reach the us input in the parameter
}
/* USER CODE END 0 */
 ...
 // HAL_Delay(100);
  ...
  while (1)
  {
	...
	//  HAL_TIM_Base_Start(&htim1);
	 ...
	 // HAL_TIM_Base_Start_IT(&htim2);
	  delay_us(10000);// 100us after tim2 will be start
	   HAL_TIM_Base_Start_IT(&htim2);
	  HAL_GPIO_TogglePin(GPIOA,GPIO_PIN_3);
      delay_ns(1000); // 10ns delay
